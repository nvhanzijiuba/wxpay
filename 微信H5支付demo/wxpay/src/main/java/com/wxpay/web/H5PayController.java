...
public class H5PayController {
    ...
    public String h5pay(@RequestParam String ip,HttpServletRequest request, HttpServletResponse response) {//InfoDTO product, ModelMap map
        // mweb_url为拉起微信支付收银台的中间页面，可通过访问该url来拉起微信客户端，完成支付,mweb_url的有效期为5分钟。
        Map<String,String> map =new HashMap<>();
        String mweb_url = wxPayDao.weixinPayH5(ip,request);
        if (StringUtils.isNotBlank(mweb_url)) {
//            return "redirect:" + mweb_url;
            return  mweb_url;
        } else {
//            return "redirect:https://blog.52itstyle.com";// 自定义错误页面
            return "https://blog.52itstyle.com";// 自定义错误页面
        }
    }

    //H5支付
    @RequestMapping(value = "/h5pays", method = RequestMethod.GET)
    public String h5spay(@RequestParam String ip, HttpServletRequest request) {//InfoDTO product, ModelMap map
        // mweb_url为拉起微信支付收银台的中间页面，可通过访问该url来拉起微信客户端，完成支付,mweb_url的有效期为5分钟。

        String aNative = wxPayDao.weixinPayH5(ip,request);
